...
export default function AssessmentPage() {
      ...
      const res = await fetch('/api/assessment', {
        ...
        body: JSON.stringify({
          ...
          answers: {
            ...
            symptoms: answers.dermalMarkers,
            diet: answers.diet,
            sunlight: answers.cognitiveClarity,
            exercise: answers.muscleRecovery,
          },
        }),
      })
  ...
  return (
    <div className="min-h-screen bg-[#0A0F14] overflow-x-hidden flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 items-center">
        ...
        {/* ── LEFT: Hero ── */}
        <div
          className="relative flex items-center justify-center px-8 py-16 lg:min-h-full text-center"
          style={{
        ...
        >
          ...
          <div className="relative z-10 max-w-md w-full">
            <span className="border border-emerald-500/40 bg-emerald-500/10 text-emerald-400 text-xs tracking-widest uppercase rounded-full px-4 py-1 inline-flex items-center gap-2">
              <FlaskConical size={12} />
              CLINICAL DEFICIENCY ASSESSMENT
            </span>

            <h1 className="mt-4 font-black text-4xl md:text-5xl lg:text-6xl max-w-5xl mx-auto leading-tight text-center">
  ...
  <span className="text-[#00E676] block">Nutrient Deficiencies</span>
        ...
        {/* ── RIGHT: Form ── */}
        <div className="flex flex-col items-center justify-center px-4 py-12 bg-[#0A0F14]">
          <div className="w-full max-w-xl">
        <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">

          {isLoading ? (
            <div className="py-16 px-8 text-center">
              <div className="w-20 h-20 rounded-full border-4 border-emerald-100 mx-auto relative">
                <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-emerald-500 animate-spin" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <Loader2 size={32} className="text-emerald-500 animate-spin" />
                </div>
              </div>
              <h2 className="mt-6 text-gray-900 font-bold text-xl">Analyzing Your Biology...</h2>
            ...
            <>
              ...
              <div className="bg-gray-50 border-b border-gray-100 px-8 py-4">
                <div className="flex justify-between items-center">
                  <span className="text-gray-500 text-sm font-medium">Step {currentStep} of 6</span>
                  <span className="text-emerald-600 text-sm font-semibold">
                    {Math.round((currentStep / 6) * 100)}% Complete
                  </span>
                </div>
                <div className="bg-gray-200 rounded-full h-1.5 w-full mt-3">
                  <div
              ...
              <div className="px-8 py-8 overflow-hidden">
                <AnimatePresence mode="wait" custom={direction}>
                  ...
                  >
                    ...
                    {currentStep === 1 && (
                      <div>
                        <span className="inline-flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-full px-3 py-1 mb-4 text-blue-600 text-xs font-semibold">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {energyOptions.map(opt => (
                            <button
                              ...
                              className={`cursor-pointer rounded-2xl border-2 p-4 transition-all duration-200 flex items-start gap-3 text-left ${
                                answers.metabolicRhythm === opt.value
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {sleepOptions.map(opt => (
                            <button
                              ...
                              className={`cursor-pointer rounded-2xl border-2 p-4 transition-all duration-200 flex items-start gap-3 text-left ${
                                answers.sleepArchitecture === opt.value
                                  ? 'border-emerald-500 bg-emerald-50'
                                  : 'border-gray-200 bg-white hover:border-emerald-300 hover:bg-emerald-50/30'
                              }`}
                            >
                    ...
                    {currentStep === 3 && (
                      <div>
                        ...
                        <p className="text-gray-400 text-sm mb-6">Select all that apply — even minor signs count</p>
                          ...
                          {dermalOptions.map(opt => {
                            ...
                            return (
                              <button
                                ...
                                className={`cursor-pointer rounded-2xl border-2 p-3 transition-all duration-200 relative text-left ${
                                  selected
                                    ? 'border-emerald-500 bg-emerald-50'
                              ...
                              >
                                {selected && (
                                  <div className="absolute top-2 right-2 bg-emerald-500 rounded-full w-5 h-5 flex items-center justify-center">
                                    <span className="text-white text-xs">✓</span>
                                  </div>
                                )}
                                <div className="text-xl mb-1">{opt.emoji}</div>
                                <div className="text-gray-800 text-sm font-medium leading-tight">{opt.label}</div>
                              </button>
                            )
                          })}
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {clarityOptions.map(opt => (
                            <button
                              ...
                              className={`cursor-pointer rounded-2xl border-2 p-4 transition-all duration-200 flex items-start gap-3 text-left ${
                                answers.cognitiveClarity === opt.value
                                  ? 'border-emerald-500 bg-emerald-50'
                                  : 'border-gray-200 bg-white hover:border-emerald-300 hover:bg-emerald-50/30'
                              }`}
                            >
                          ...
                          <div className="flex flex-wrap gap-2">
                            {muscleOptions.map(opt => (
                              <button
                                ...
                                className={`border-2 rounded-full px-4 py-2 text-sm cursor-pointer font-medium transition-all ${
                                  answers.muscleRecovery === opt.value
                          ...
                          <div className="flex flex-wrap gap-2">
                            {immuneOptions.map(opt => (
                              <button
                                ...
                                className={`border-2 rounded-full px-4 py-2 text-sm cursor-pointer font-medium transition-all ${
                                  answers.immuneResilience === opt.value
                                    ? 'bg-emerald-500 text-black border-emerald-500'
                                    : 'border-gray-300 text-gray-600 hover:border-emerald-400'
                                }`}
                              >
                    ...
                    {currentStep === 6 && (
                      <div>
                        ...
                        <div className="flex flex-col gap-4">
                          <div>
                            <label className="block text-gray-700 text-sm font-medium mb-1">Your first name</label>
                            <input
                              type="text"
                              placeholder="e.g. Priya"
                              value={answers.name}
                              onChange={e => setAnswers(prev => ({ ...prev, name: e.target.value }))}
                              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-900 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100 transition bg-white"
                            />
                          </div>

                          <div>
                            <label className="block text-gray-700 text-sm font-medium mb-1">Your age</label>
                            <input
                              type="number"
                              placeholder="e.g. 28"
                              min={10}
                          ...
                          <div>
                            <label className="block text-gray-700 text-sm font-medium mb-1">Primary health goal</label>
                            <select
                              value={answers.goal}
              ...
              <div className="px-8 pb-8 flex items-center justify-between">
                <button
                  ...
                  className={`border border-gray-200 text-gray-500 rounded-full px-6 py-3 hover:border-gray-400 transition flex items-center gap-2 text-sm font-medium ${currentStep === 1 ? 'invisible' : ''}`}
                >
                  <ChevronLeft size={16} />
                  Back
                </button>
                ...
                {currentStep < 6 ? (
                  <button
                    ...
                    className={`bg-emerald-500 text-black rounded-full px-8 py-3 font-bold text-sm flex items-center gap-2 transition-all ${
                      isStepValid()
