...
export function LockedPremiumOffer({
  ...
  return (
    <motion.section
      ...
      viewport={{ once: true }}
      className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-[#111810] p-5 sm:p-8 lg:p-10"
    >
      ...
      <div className="relative lg:grid lg:grid-cols-2 lg:items-center lg:gap-12">
        <div>
          ...
          <p className="mt-3 max-w-md text-sm leading-relaxed text-gray-400 lg:text-base">
          ...
          <div className="relative mt-6 flex items-end gap-3">
            ...
            <p className="text-5xl font-black leading-none text-white lg:text-6xl">₹39</p>
          ...
          <button
            ...
            onClick={onUnlock}
            disabled={unlocking}
            className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-emerald-500 py-4 text-base font-black text-black shadow-[0_0_24px_rgba(16,185,129,0.28)] transition hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60 lg:max-w-sm"
          >
            {unlocking ? 'Loading…' : 'Unlock my plan — ₹39'}
            <ChevronRight size={18} />
          </button>
          {error ? <p className="mt-3 text-center text-sm font-medium text-red-400 lg:text-left">{error}</p> : null}
          <p className="mt-3 text-center text-xs text-gray-500 lg:text-left">Private · Yours in minutes · No guesswork</p>
        </div>
        ...
        <div className="relative mt-6 lg:mt-0">
          <div className="space-y-2.5">
            {LOCKED_ITEMS.map(({ Icon, title, tease }) => (
              <div
                key={title}
                className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-black/30 px-3.5 py-3 lg:px-4 lg:py-3.5"
              >
          ...
          <div className="pointer-events-none absolute inset-0 rounded-xl bg-gradient-to-t from-[#111810] via-[#111810]/40 to-transparent lg:bg-gradient-to-l" />
        </div>
      </div>
    </motion.section>
  )
