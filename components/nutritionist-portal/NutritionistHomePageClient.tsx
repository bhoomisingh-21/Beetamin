...
export default function NutritionistHomePageClient({ initial }: { initial: PortalHomePayload }) {
  ...
  function markDone(id: string) {
    start(async () => {
      await completePortalAppointment(id, '')
      refresh()
    })
  }
            ...
            {todaySessions.map((a) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {a.status === 'completed' && (
                    <span className="inline-flex items-center gap-1 rounded-xl border border-blue-500/30 px-4 py-2.5 text-sm font-bold text-blue-300">
                      <CheckCircle size={16} />
                      Done
                    </span>
                  )}
